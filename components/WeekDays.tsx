...
export const WeekDays = () => {
    ...
    return (
        <View>
            ...
            <View className='w-full h-fit bg-zinc-900 rounded-3xl mt-2 mb-20 px-3 py-5 flex gap-7 shadow-lg shadow-zinc-800 border border-zinc-70'>
                {weekDats.map((day, index) => {
                    const isEmpty = routine?.days.find(d => d.day === day.name.toLowerCase()) === undefined;
                    return (
                        <Link asChild href={`/routine/dayRoutine?day=${encodeURIComponent(day.name)}`} key={index}>
                            <Pressable className="w-full flex flex-row items-center justify-between border border-zinc-700/40 rounded-3xl px-5 py-4 gap-2">
                                ...
                                <Entypo name="dot-single" size={24} color="white" />
                                    <Text className="text-zinc-300 font-semibold text-xl">{day.name}: <Text className='text-zinc-400 text-[14px]'>{isEmpty ? 'vacio' : 'creado'}</Text></Text>
                                </View>
